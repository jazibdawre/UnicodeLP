...
projectRouter
	...
	.post((req, res, next) => {
		res.statusCode = 403;
		res.end(
			'POST operation not supported on /projects/' + req.params.projectId
		);
	})
	.put(authenticate.verifyUser, async (req, res, next) => {
		try {
			...
			); //new returns the modified project data
			res.statusCode = 200;
			res.setHeader('Content-Type', 'application/json');
			res.json(project);
		} catch (error) {
	...
	.delete(
		...
		async (req, res, next) => {
			try {
				const resp = await Projects.findByIdAndRemove(
					req.params.projectId
				);
				res.statusCode = 200;
				res.setHeader('Content-Type', 'application/json');
